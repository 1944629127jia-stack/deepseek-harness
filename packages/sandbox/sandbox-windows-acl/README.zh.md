...
- **控制台隔离不可用。** 在受限令牌下，以 `CREATE_NO_WINDOW` / `CREATE_NEW_CONSOLE` 创建的子进程在 DLL 初始化期间以 `STATUS_DLL_INIT_FAILED`（`0xC0000142`）死亡。POC 尝试把控制台登录 SID（`S-1-2-1`）加入 restricting 列表来修复；在 Windows 11 26200 上 `CreateWellKnownSid(WinLocalLogonSid)` 以 `ERROR_INVALID_PARAMETER`（87）失败，正确的 `WinConsoleLogonSid` 能产出合法 `S-1-2-1` 但子进程仍然死亡，POC 的最终修订同时移除了该 SID 与控制台隔离。子进程因此共享宿主控制台；stdio 重定向走管道，不受影响。**无控制台的宿主必须先获得一个控制台**（`ensureHostConsole`，由 `dsh-sandbox-local` 在每次 windows-acl 包装前调用）：在打包的桌面外壳（宿主进程树中没有任何控制台）里，每个初始化需要自己创建控制台连接的受限子进程都以同样方式死在 KernelBase 的 `DLL_PROCESS_ATTACH` 里，而 `DETACHED_PROCESS` 子进程虽能启动，PowerShell 却不执行命令空转退出。每个宿主进程生命周期只需一个隐藏控制台，即可覆盖它派生的所有 runner 与子进程。由于打包外壳把 runner 本身也作为 GUI 子系统 Electron 映像运行——它在创建时从不继承宿主控制台——runner 还会在启动时附加到父进程控制台（`attachParentConsole`），使受限子进程无论 runner 映像的子系统如何，始终有控制台可继承。
...
- **受限孙进程的管道 stdio 捕获不可用（named pipe 的默认 SD 模板）。** libuv 的管道 stdio 用的是 NAMED pipe；不带安全属性调用 `CreateNamedPipeW` 时，其默认安全描述符不是内核的模板，而是 Win32 层在用户态安装的默认 SD 模板（由 KernelBase 构建——owner/SYSTEM/Admins 全权，Everyone/ANONYMOUS 只读，即 [MS 文档](https://learn.microsoft.com/en-us/windows/win32/ipc/named-pipe-security-and-access-rights)记载的固定模板）——**不是**令牌默认 DACL（后者才是内核在原始 SD-null 创建时应用的）——因此 client 端打开所请求的写访问没有任何 restricting SID 被授予：受限进程内 `spawn(..., { stdio: 'pipe' })` 以 EPERM 失败，这是 POC 记载的 WRITE_RESTRICTED「无法重定向输出」边界。继承（`inherit`/fd）与忽略（`ignore`）stdio 的 spawn 可用；匿名管道（CreatePipe——令牌默认 DACL 的消费者，例如 PowerShell 的管道与 .NET Framework 的 `ProcessStartInfo` 重定向）因受限令牌默认 DACL 同时携带 restricting SID 与 Everyone 的全权 ACE（init 时写入）而可用：capability ACE 只能满足 pass-2（受限）检查，永远无法满足 pass-1——capability SID 是 restricting SID 而非令牌组——因此在继承的默认 DACL 未授予创建者写权限的宿主上（Explorer 启动的 GUI 应用只带登录 SID 的 `GX|GR`，不像终端启动的 shell 带 `LA GA`），受限子进程内的匿名管道创建会以 ERROR_ACCESS_DENIED 死亡（打包桌面外壳的管道孙进程 MSB3883）；Everyone ACE 既是启用组又是 restricting SID，在任何宿主祖先链下都能同时覆盖两项检查。受限进程因此无法用管道捕获孙进程输出；必须捕获输出的工具无法在受限下运行。
